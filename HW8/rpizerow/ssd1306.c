...
void drawChar(uint x, uint y, unsigned char letter){
    for (int i = 0; i < 5 ; i++){
...
void ssd1306_drawString(int x_start, int y_start, volatile char * m){
    int i = 0;
    uint x_pos = x_start;
    uint y_pos = y_start;
    while(m[i] != '\0'){
        if(m[i] == '\r'){
            ...
            //x_pos = x_start + (i*5);
            drawChar(x_pos, y_pos, m[i]);
            if(((x_pos + 5) > 128) && ((y_pos + 8) > 32)){
                x_pos = x_start;
                y_pos = y_start;
            }else if(((x_pos + 5) > 128) && ((y_pos + 8) < 32)){
                x_pos = x_start;
                y_pos += 8;
            }else{
                x_pos += 5;
